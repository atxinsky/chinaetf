...
def render_data_update():
    ...
    from config import ETF_POOLS, ALL_ETFS

    col1, col2 = st.columns(2)

    with col1:
        # 选择ETF池
        pool_options = ["全部"] + list(ETF_POOLS.keys())
        ...
        try:
            ...
            for i, code in enumerate(selected_codes):
                ...
                try:
                    ...
                    results.append({"代码": code, "名称": ALL_ETFS.get(code, ""), "新增条数": rows, "状态": "✅ 成功"})
                except Exception as e:
                    results.append({"代码": code, "名称": ALL_ETFS.get(code, ""), "新增条数": 0, "状态": f"❌ 失败: {e}"})
            ...
            total_rows = sum([r["新增条数"] for r in results])
...
def render_data_stats():
    ...
    try:
        ...
        st.download_button("📥 下载统计数据", csv, "data_stats.csv", "text/csv")
